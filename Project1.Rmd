...
ggplot(industry_data, aes(x = manufacturing_idx, y = hospitality_idx)) +
  geom_point() +
  theme_minimal() +
  labs(x = "Manufacturing Index", y = "Hospitality Index", title = "Relationship between Two Industries")
...
```{r}
daily$solar_radiation_category <- cut(x = daily$solar_radiation, 
                                      breaks = quantile(daily$solar_radiation, 
                                                        probs = c(0, 1/3, 2/3, 1), na.rm = TRUE), 
                                      include.lowest = TRUE, 
                                      labels = c('Low', 'Medium', 'High'))

grouped_data <- group_by(daily, solar_radiation_category)
...
The average consumption for small consumers is highest in the low solar radiation category and significantly decreases as solar radiation increases. This pattern may indicate that on days with stronger solar radiation, small consumers may prefer to use solar power or other alternative sources of energy instead of natural gas, thereby reducing natural gas consumption.

Industrial consumption is also highest in the low solar radiation category, but the decreasing trend is less pronounced compared to small consumers. This might reflect a more stable energy demand in the industrial sector that is less influenced by changes in solar radiation, or it could suggest that the industry has a more diversified energy mix.

Average consumption for power consumption is highest when solar radiation is low and lowest when solar radiation is high. This could imply that the power sector is able to rely more on solar energy production during times of high solar radiation, thus reducing dependency on natural gas.

From this investigation, we can speculate that natural gas consumption may be influenced by the level of solar radiation, especially among small consumers and the power sector. Higher levels of solar radiation tend to coincide with lower levels of natural gas consumption, possibly due to an increased use of renewable energy sources like solar power. However, this effect seems less significant for industrial consumption, possibly due to the stability of energy demands in that sector or due to a more diversified use of different forms of energy.
