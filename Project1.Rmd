...
```{r}
daily$heating_profiles_category <- cut(daily$heating_profiles,
                                       breaks=quantile(daily$heating_profiles, 
                                                       probs = c(0, 1/3, 2/3, 1), na.rm = TRUE),
                                       labels=c("Low", "Medium", "High"),
                                       include.lowest=TRUE)

avg_consumption_small <- aggregate(consumption_small ~ heating_profiles_category, data=daily, FUN=mean, na.rm=TRUE)

avg_consumption_industry <- aggregate(consumption_industry ~ heating_profiles_category, data=daily, FUN=mean, na.rm=TRUE)

avg_consumption_power <- aggregate(consumption_power ~ heating_profiles_category, data=daily, FUN=mean, na.rm=TRUE)

combined_results <- data.frame(
  Heating_Profile_Category = avg_consumption_small$heating_profiles_category,
  AvgConsSmall = avg_consumption_small$consumption_small,
  AvgConsIndus = avg_consumption_industry$consumption_industry,
  AvgConsPower=avg_consumption_power$consumption_power
)
...
What I have learned:
1. Positive correlation: there is a clear trend showing that as the heating profile category increases from "Low" to "High," there is a corresponding increase in energy consumption across small consumption, industry consumption, and power consumption.
2. Industry Stability: The industry consumption shows a relatively stable consumption pattern compared to small consumption and power consumption, suggesting that industrial energy use may be less influenced by the factors driving the heating profiles, such as weather conditions or heating efficiency.
3.Significant Increase in Power Consumption: The power consumption displays the most significant increase in consumption with higher heating profiles, indicating that electricity demand for heating is greatly influenced by the heating requirements.
