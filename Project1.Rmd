...
```         
After some sorting through data, we can see that there is indeed a pattern to which rows have
missing values across these six columns. Columns price_industry_idx, price_households_idx and
consumption_small all have 365 missing values in total and they all come from rows 1-365.
Columns consumption_industry and consumption_power both only have 1 missing value and for both
columns, that missing value is in row 2052. Lastly, column wages has 89 missing values in rows
...
-   (FIONA) Limit the `gas` table to days where the price exceeded the yearly median. Use the concept of circular means to compute the average day of the year when price exceeds the yearly median price. The `yday` function will likely be useful here.
...
-   (KAELYN) Explore the [documentation for ggplot](https://ggplot2.tidyverse.org/). Select one geometry and one `stat_` function we have not used before or use an option to a previously used geometry/stat with a new option. Write a short paragraph explaining what the plots show.
-   (FIONA) Investigate solar radiation's marginal distribution and also it's relationship with temperature.
...
Increased Energy Use in Colder Weather: The highest average consumption across all sectors
during low temperatures likely reflects increased heating requirements.

Stable Industrial Consumption: Industrial consumption changes less dramatically across
temperature ranges, which may indicate that industrial operations maintain a relatively
constant energy use, regardless of weather conditions.

Reduced Energy Use in Warmer Weather: The noticeable decrease in energy consumption for small
consumption and power consumption as temperatures rise suggests that there may be less reliance
on energy-intensive heating systems, and possibly more efficient cooling systems or less need
for active cooling.
```
...
```         
How have gas prices changed in the time since this data was collected? We can see that towards
the end of 2022, natural gas prices were beginning to increase. It would be interesting to
examine whether this trend has continued in the years following.

Although we found there to be trends regarding missing values and in what rows they are found,
we are still unsure as to why these patterns occur. What is the reasoning/context behind these
patterns, and what is unique about these rows that seem to contain more missing values than
others?

We found that the average daily energy consumption was slightly more on non-holidays as opposed
to holidays. How does gas price factor into this relationship—does gas cost, on average, the
same on holidays and nonholidays, or different? And how do holidays affect gas prices in
...
```         
This project aimed to analyze the effects of the energy crisis in Europe, specifically in
Germany, in the years after Russia limited its gas exports and how German gas consumers
responded to/were impacted by those effects. Although there were a vast amount of tests run on
this data, some key takeaway results were that there was a prominent increase in gas prices in
2021 when Russia curbed their gas exports to Europe and that during the crisis, energy
consumption decreased. The results of the analysis are significant and should be of interest to
potential readers as they show a big picture of how closely different variables are related and
how one change in an economic good can impact the daily lives of consumers, as they adjust to
new prices and limited availability of that good. Thus, in a broad sense, this project, and the
tests that were run in it, can be applied to a change in any consumer good price/availability.
```
...
```         
For this project, we divided up each of the 16 tasks and objective equally among our group
members and each took time on our own to work on them. After doing our best each on our own, we
got together to put all of our work together and check each other's work. In this meeting, we
also worked to fill out the last couple questions, concerning the dataset as a whole, and what
we can learn from/ask more about it. By doing the project in this way, we ensured that everyone
did an equal amount of work, but we were still collaborative and used all of our skillsets to
their advantages.
```
